...
#include "geos_funcs_YYd_b.h"
#include "geos_funcs_Y_O.h"
#include "geos_funcs_YY_O.h"
#include "geos_funcs_YYO_b.h"
#include "geos_funcs_Ydi_Y.h"
#include "geos_funcs_Ydddd_Y.h"
...
#define GEOS_VERSION_PATCH_STR QUOTE(GEOS_VERSION_PATCH)
  ...
  if (init_geos_funcs_YYd_b(m, d) < 0) {
    return NULL;
  };

  if (init_geos_funcs_Y_O(m, d) < 0) {
    return NULL;
  };

  if (init_geos_funcs_YY_O(m, d) < 0) {
    return NULL;
  };

  if (init_geos_funcs_YYO_b(m, d) < 0) {
    return NULL;
  };

  if (init_geos_funcs_Ydi_Y(m, d) < 0) {
    return NULL;
  };

  if (init_geos_funcs_Ydddd_Y(m, d) < 0) {
    return NULL;
  };
