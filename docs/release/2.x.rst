...
universal functions), the performance of the scalar geometry objects and
their attributes and methods did not see improvements, or even slight
regressions because of additional overhead (#2035, #2339).

With shapely 2.2, we also improved the performance of scalar function calls
significantly. For example, considering a single ``Point`` geometry (``p =
shapely.Point(1.0, 2.0)``), accessing a cheap attribute such as its
x-coordinate (``p.x``) now takes around 50-100 nanoseconds instead of 2-3
microseconds.

Many of the other attributes and methods have also received performance
improvements. This is achieved by a combination of specialized scalar versions
(avoiding the overhead of the NumPy ufunc machinery) and improved GEOS context
handling (instead of initializing/finalizing on every call, GEOS contexts are
reused through a thread-local variable).
...
- :func:`.coverage_clean`, which returns a cleaned version of geometries
  forming a polygonal coverage (#2454)  (requires GEOS >= 3.14).
- :func:`~shapely.split` to split a geometry by another geometry (uses
  GEOS>=3.15, falls back to the existing :func:`shapely.ops.split`
  implementation otherwise) (#2477).
...
- :func:`.Polygon.from_bounds` now builds the ring in counterclockwise order,
  consistent with :func:`shapely.box`, so the result satisfies the GeoJSON
...
- The :func:`~shapely.split` and :func:`shapely.ops.split` functions now
  support splitting a (Multi)Polygon by another (Multi)Polygon, by using the
  boundary to split to with (similarly as was already done for splitting a
  (Multi)LineString with a polygon) (#2505).

API changes
^^^^^^^^^^^

- The :func:`shapely.ops.transform` is deprecated. Use the more specific
  :meth:`shapely.transform` (when your function works independently on all
  coordinates) or :meth:`shapely.transform_coordseq` (when your function works
  on the coordinate sequence directly) instead (#1902).
...
- Shapely 2.2.0 requires GEOS >= 3.10, Python >= 3.11, and NumPy >= 1.26
  (#2302, #2301, #2499).
- Shapely now uses ``meson-python`` for its build backend (#2357).
