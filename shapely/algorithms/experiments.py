...
def run_comparison_experiment(num_trials: int, output_file: str):
    ...
    with open(output_file, 'w', newline='') as csvfile:
        fieldnames = ['trial', 'before_time', 'before_length', 'after_time', 'after_length']
        ...
        for instance in range(num_trials):
            ...
            logger.info(f"Before: Runtime = {run_time_before}, Length = {partition_length_before}")
            logger.info(f"After: Runtime = {run_time_after}, Length = {partition_length_after}")
...
if __name__ == "__main__":
    results = run_comparison_experiment(3, "comparison_results.csv")
    plot_comparison(results)
    print("Experiment completed. Results saved in 'comparison_results.csv' and 'performance_comparison.png'.")