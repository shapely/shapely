...
@pytest.mark.parametrize("geom", all_types)
@mock.patch("shapely.io.ShapelyGeometry", ShapelyGeometryMock)
@mock.patch("shapely.io.ShapelyPreparedGeometry", ShapelyPreparedMock)
@mock.patch("shapely.io.shapely_compatible", True)
@mock.patch("shapely.io._shapely_checked", True)
def test_from_shapely(geom):
    actual = shapely.from_shapely(ShapelyGeometryMock(geom))
...
def test_from_shapely_prepared(geom):
    actual = shapely.from_shapely(ShapelyPreparedMock(geom))
    assert isinstance(actual, shapely.Geometry)
    assert_geometries_equal(geom, actual)
    assert geom._ptr != actual._ptr
...
def test_from_shapely_arr():
    actual = shapely.from_shapely([ShapelyGeometryMock(point), None])
...
def test_from_shapely_none():
    actual = shapely.from_shapely(None)
...
def test_from_shapely_error(geom):
    with pytest.raises(TypeError):
        shapely.from_shapely(geom)


@pytest.mark.parametrize("geom", all_types)
@mock.patch("shapely.io.ShapelyGeometry", ShapelyGeometryMock)
@mock.patch("shapely.io.ShapelyPreparedGeometry", ShapelyPreparedMock)
@mock.patch("shapely.io.shapely_compatible", False)
@mock.patch("shapely.io._shapely_checked", True)
...
@pytest.mark.parametrize("geom", all_types)
@mock.patch("shapely.io.ShapelyGeometry", ShapelyGeometryMock)
@mock.patch("shapely.io.shapely_wkb_loads", shapely_wkb_loads_mock)
@mock.patch("shapely.io.shapely_compatible", False)
@mock.patch("shapely.io._shapely_checked", True)
def test_to_shapely_incompatible(geom):
    if shapely.get_type_id(geom) == shapely.GeometryType.LINEARRING:
        pytest.skip("Linearrings are not preserved in WKB")
    actual = shapely.to_shapely(geom)
...
def test_to_shapely_incompatible_none():
    actual = shapely.to_shapely(None)
    assert actual is None
...
def test_to_shapely_incompatible_array():
    actual = shapely.to_shapely([point, None])
