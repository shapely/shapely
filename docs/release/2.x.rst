...
- :func:`.split` to split a geometry by another geometry (uses GEOS>=3.15,
  falls back to the existing :func:`shapely.ops.split` implementation
  otherwise) (#2477).
...
- The :func:`.split` and :func:`shapely.ops.split` functions now support
  splitting a (Multi)Polygon by another (Multi)Polygon, by using the boundary
  to split to with (similarly as was already done for splitting a
