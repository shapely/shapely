...
__all__ = ["to_coordinates_offsets", "from_coordinates_offsets"]
...
def to_coordinates_offsets(arr):
    """
...
def _multipoint_from_flatcoords(coords, offsets):
    ...
    result = creation.multipoints(points, indices=multipoint_indices)
...
def _linestring_from_flatcoords(coords, offsets):
    ...
    result = creation.linestrings(coords.reshape(-1, 2), indices=linestring_indices)
...
def _multilinestrings_from_flatcoords(coords, offsets1, offsets2):
    ...
    result = creation.multilinestrings(linestrings, indices=multilinestring_indices)
...
def _polygon_from_flatcoords(coords, offsets1, offsets2):
    ...
    result = creation.polygons(rings, indices=polygon_indices)
...
def _multipolygons_from_flatcoords(coords, offsets1, offsets2, offsets3):
    ...
    result = creation.multipolygons(polygons, indices=multipolygon_indices)

    return result


def from_coordinates_offsets(typ, coords, offsets=None):
    """
