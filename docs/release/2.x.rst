...
help to validate an array of geometries as valid topological coverage and
inspect invalid edges. The :func:`.coverage_simplify` function then allows
topological simplification of polygonal coverages (in contrast to the existing
:func:`.simplify` funciton, which simplifies an array of geometries one by one,
independently).
